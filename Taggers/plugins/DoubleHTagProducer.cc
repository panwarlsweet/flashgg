        ...
        for( unsigned int candIndex = 0; candIndex < diPhotons->size() ; candIndex++ ) {
            ...
            if( ! evt.isRealData() ) {
                ...
                if( selgenjetsnu.size() >= 2 && selgenjets_nonu.size() >= 2) {
                    ...
                    if ( dRmin1 > 0.3 || dRmin2 > 0.3 ) continue;
                    if ( ( dRmin1 == dR11 && dRmin2 == dR12) ||  ( dRmin1 == dR21 && dRmin2 == dR22 ) ) continue;
                    if ( (dRmin1 == dR11 && dRmin2 == dR22 ) || (dRmin1 == dR21 && dRmin2 == dR12 ))
                        {
                            ...
                            MRegVars["recoJet_1_pt"] = leadJet->p4().pt();
                            MRegVars["recoJet_1_eta"] = leadJet->p4().eta();
                            MRegVars["recoJet_1_phi"] = leadJet->p4().phi();
                            MRegVars["recoJet_1_e"] = leadJet->p4().energy();
                            MRegVars["recoJet_1_mass"] = leadJet->p4().M();
                            MRegVars["recoJet_1_DeepCSV"] = leadJet->bDiscriminator("pfDeepCSVJetTags:probb") + leadJet->bDiscriminator("pfDeepCSVJetTags:probbb");
                            MRegVars["recoJet_1_DeepJet"] = leadJet->bDiscriminator("mini_pfDeepFlavourJetTags:probb") + leadJet->bDiscriminator("mini_pfDeepFlavourJetTags:probbb") + leadJet->bDiscriminator("mini_pfDeepFlavourJetTags:problepb");
                            MRegVars["recoJet_2_pt"] = subleadJet->p4().pt();
                            MRegVars["recoJet_2_eta"] = subleadJet->p4().eta();
                            MRegVars["recoJet_2_phi"] = subleadJet->p4().phi();
                            MRegVars["recoJet_2_e"] = subleadJet->p4().energy();
                            MRegVars["recoJet_2_mass"] = subleadJet->p4().M();
                            MRegVars["recoJet_2_DeepCSV"] = subleadJet->bDiscriminator("pfDeepCSVJetTags:probb") + subleadJet->bDiscriminator("pfDeepCSVJetTags:probbb");
                            MRegVars["recoJet_2_DeepJet"] = subleadJet->bDiscriminator("mini_pfDeepFlavourJetTags:probb") + subleadJet->bDiscriminator("mini_pfDeepFlavourJetTags:probbb") + subleadJet->bDiscriminator("mini_pfDeepFlavourJetTags:problepb");
                            MRegVars["reco_Mbbgg"] = (leadJet->p4() + subleadJet->p4() + dipho->p4()).M();
                            MRegVars["reco_mjj"] = (leadJet->p4() + subleadJet->p4() ).M();       
            
                            edm::Handle<View<flashgg::Met> > mets;
                            evt.getByToken( METToken_, mets );
                            if( mets->size() != 1 )
                                { std::cout << "WARNING number of MET is not equal to 1" << std::endl; }
                            Ptr<flashgg::Met> met = mets->ptrAt( 0 );
                            MRegVars["reco_MET"] = met->p4().pt();
                            MRegVars["reco_phiMET"] = met->p4().phi();
                        }
            ...
            if (dottHTagger_) 
            {
                ...
                tag_obj.recoJet_1_pt_ = MRegVars["recoJet_1_pt"];
                tag_obj.recoJet_1_eta_ = MRegVars["recoJet_1_eta"];
                tag_obj.recoJet_1_phi_ = MRegVars["recoJet_1_phi"];
                tag_obj.recoJet_1_e_ = MRegVars["recoJet_1_e"];
                tag_obj.recoJet_1_mass_ = MRegVars["recoJet_1_mass"];
                tag_obj.recoJet_1_DeepCSV_ = MRegVars["recoJet_1_DeepCSV"];
                tag_obj.recoJet_1_DeepJet_ = MRegVars["recoJet_1_DeepJet"];
     
                tag_obj.recoJet_2_pt_ = MRegVars["recoJet_2_pt"];
                tag_obj.recoJet_2_eta_ = MRegVars["recoJet_2_eta"];
                tag_obj.recoJet_2_phi_ = MRegVars["recoJet_2_phi"];
                tag_obj.recoJet_2_e_ = MRegVars["recoJet_2_e"];
                tag_obj.recoJet_2_mass_ = MRegVars["recoJet_2_mass"];
                tag_obj.recoJet_2_DeepCSV_ = MRegVars["recoJet_2_DeepCSV"];
                tag_obj.recoJet_2_DeepJet_ = MRegVars["recoJet_2_DeepJet"];
                tag_obj.reco_mjj_ = MRegVars["reco_mjj"];
                tag_obj.reco_Mbbgg_ = MRegVars["reco_Mbbgg"];
                tag_obj.reco_MET_ = MRegVars["reco_MET"];
                tag_obj.reco_phiMET_ = MRegVars["reco_phiMET"];
                tag_obj.nbGenJetsNu_ = MRegVars["nbGenJetsNu"];
                tag_obj.nGenJets_ = MRegVars["nGenJets"];
                tag_obj.nNus_ = MRegVars["nNus"];
